        ...
        <section className="font-pop dark:bg-gray-950 ">
            ...
            <Navbar />
                ...
                <div className="  animate-appear3 flex gap-2 mt-5 mb-5 px-15 font-mont font-light ">
                    <button onClick={() => setShowEventModal(true)} className="rounded-sm bg-blue text-white p-1.5 flex items-center justify-center border-blue border-1 hover:text-blue hover:border-blue hover:bg-white cursor-pointer">
                        Create New Event
                    </button>
                ...
                <div className="z-0 dark:inset-shadow-none inset-shadow-sm inset-shadow-gray-500 py-10 justify-between grid grid-cols-3 gap-y-0 place-items-center w-full px-25">
                     {events.map((event, index) => (
                         <AdminCard onEditClick={onEditClick} onEventUpdated={fetchEvents} key={event.event.id} event={event} index={index} />
                     ))}
                </div>
            </section>
        </>
